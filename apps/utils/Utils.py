    ...
    def SetEntities(self, states, state):
        for entity in states[state]:
            ...
            if domain == "input_select":
                ...
                self.call_service(
                    "media_player/turn_" + entity["state"], entity_id=entity["entity"]
                )
                self.log(f'Set {entity["entity"]} to {entity["state"]}.')
            elif domain == "climate":
                self.call_service(
                ...
                self.call_service(
                    "switch/turn_" + entity["state"], entity_id=entity["entity"]
                )
                self.log(f'Set {entity["entity"]} to {entity["state"]}.')
            elif domain == "fan":
                self.call_service(
                    "fan/turn_" + entity["state"], entity_id=entity["entity"]
                )
                self.log(f'Set {entity["entity"]} to {entity["state"]}.')
            elif domain == "light":
                ...
                if entity["state"] == "on":
                    ...
                    self.call_service(
                        "light/turn_" + entity["state"], entity_id=entity["entity"]
                    )
                self.log(f'Set {entity["entity"]} to {entity["state"]}.')
