...
class MediaPlayer(hass.Hass):
  def initialize(self):
    ...
    self.adjust_volume = self.args['adjust_volume'] if 'adjust_volume' in self.args.keys() else None
    self.remote_device = self.args['remote_device'] if 'remote_device' in self.args.keys() else None
  ...
  def TurnOffLinked(self, kwargs):
    ...
    if player_state == 'off':
      if self.remote_device:
        self.call_service('remote/send_command', entity_id=self.linked, device=self.remote_device, command='PowerOff')
      else:
  ...
  def TurnOnLinked(self):
    if self.remote_device:
      self.call_service('remote/send_command', entity_id=self.linked, device=self.remote_device, command='PowerOn')

  def SetVolume(self, kwargs):
    volume = kwargs['volume']
    self.call_service('media_player/volume_set', entity_id=self.entity, volume_level=volume)
